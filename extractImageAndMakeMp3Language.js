...
*/
  ...
  text = text.replace(/hmph!|hmph|'|"|`|\.\.|\s\.|p p|pp|”\.|“|«|»/g,"");
    ...
    const filePath = path.join(rootFolder, `mp3${language}Slow/${videoIndex}`, `${LineIndex}.mp3`);
    //const filePath = path.join(rootFolder, `mp3${language}/${videoIndex}`, `${LineIndex}.mp3`);
    await new Promise((resolve, reject) => {
    // Lấy độ dài của file
     const command = `gtts-cli --lang ${symbol} '${text}' --output "${filePath}"`;
      console.log(command);
  ...
  const fastFilePath = path.join(rootFolder, `mp3${language}/${videoIndex}`, `${LineIndex}.mp3`);
  ...
  async function downloadMP3(url, filename) {
    ...
    const filePath = path.join(rootFolder, `mp3${language}/${videoIndex}`, filename);
...
async function Run(RootFolder,VideoIndex){
    ...
    textFile = "./" +rootFolder+`/translatedSub${language}/translate${videoIndex}.txt`;
    if(!fs.existsSync(path.join(rootFolder, `mp3${language}/${videoIndex}`))) fs.mkdirSync(path.join(rootFolder, `mp3${language}/${videoIndex}`))
    else{
        fs.rmdirSync(path.join(rootFolder, `mp3${language}/${videoIndex}`), { recursive: true, force: true })
        fs.mkdirSync(path.join(rootFolder, `mp3${language}/${videoIndex}`))
    }

        if(!fs.existsSync(path.join(rootFolder, `mp3${language}Slow`))) fs.mkdirSync(path.join(rootFolder, `mp3${language}Slow`))
    else{
        fs.rmdirSync(path.join(rootFolder, `mp3${language}Slow`), { recursive: true, force: true })
        fs.mkdirSync(path.join(rootFolder, `mp3${language}Slow`))
    }
    if(!fs.existsSync(path.join(rootFolder, `mp3${language}Slow/${videoIndex}`))) fs.mkdirSync(path.join(rootFolder, `mp3${language}Slow/${videoIndex}`))
    else{
        fs.rmdirSync(path.join(rootFolder, `mp3${language}Slow/${videoIndex}`), { recursive: true, force: true })
        fs.mkdirSync(path.join(rootFolder, `mp3${language}Slow/${videoIndex}`))
    }
