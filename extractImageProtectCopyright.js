...
/*.complexFilter([
    '[0]crop=iw:ih-50:0:50,scale=iw-20:ih-20[padded]', // Giảm kích thước ảnh gốc
...
*/
.complexFilter([
    '[0]crop=iw:ih-100:0:100,scale=iw-50:ih-50[padded]', // Giảm kích thước ảnh gốc
    '[1][padded]scale2ref=w=iw+50:h=ih+50[bg][padded]', // Tăng kích thước nền
    '[bg][padded]overlay=(W-w)/2:(H-h)/2[padded]', // Căn giữa ảnh trên nền
//    '[2][padded]scale2ref=w=iw-20:h=ih-20[bg][padded]', // Tăng kích thước nền
//    '[padded][2]overlay=(W-w)/2:(H-h)/2[padded]', // Căn giữa ảnh trên nền
    '[padded][2]overlay=W-w:50[padded]', // Căn giữa ảnh trên nền
    ...
    "[draw]drawtext=text='EngSub Commics':fontsize=25:fontcolor=white@0.02:x=(w-text_w)/2:y=(h-text_h)/2[paint]", // Thêm chữ
    "color=red@0.1:size=1920x1080[overlay]", // Tạo lớp mờ đen (đặt kích thước cố định hoặc tự động)
    "[paint][overlay]overlay=format=auto" // Phủ lớp mờ lên ảnh đã qua bộ lọc khác
])
