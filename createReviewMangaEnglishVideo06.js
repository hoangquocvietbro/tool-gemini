...
async function Run() {
    ...
    try {
      ...
      try {
        try {
          ...
          await sendFileToGeminiToTranslateText(rootFolder, index, apiKeys[5]);
        }
      } catch {
        await sendFileToGeminiToTranslateText(rootFolder, index, apiKeys[7]);
      }
  ...
  console.log("Hoàn thành toàn bộ"); process.exit(1);
}
