...
const extractImageNoOrderText = require("./extractImageNoOrderText");
...
var apiKeys = process.env.API_KEY_PROCESS_SEGMENT.split("|");
...
if (args[0]) {
     ...
     try{
      if(startIndex=='saveIndex') startIndex=parseInt(fs.readFileSync("./" + args[2].replace(/^-+/, '') +"/saveIndex.txt",'utf-8'))+1;
     }
     catch{
      startIndex = 0;
     }
...
let sliceApiIndex= 0
if (args[3]) {
     sliceApiIndex = args[3]; // lấy tham số thứ hai
     apiKeys = process.env.API_KEY_PROCESS_SEGMENT.split("|").slice(sliceApiIndex);
}
  const urls = fs
...
async function Run(apiKeys, startIndex = 0,condition=false,endIndex=10,colors,colorIndex, console = {}) {
  ...
  if (args[2]) {
     rootFolder = args[2].replace(/^-+/, ''); // lấy tham số thứ hai
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if(!fs.existsSync("./" + rootFolder + `/originVideo/videoplayback.mp4`)&&!fs.existsSync("./" + rootFolder + `/originVideo/videoHightQuality.mp4`)){
      console.log("Bắt đâu download và cut video")
        await Promise.all([
            longYoutubeVideoDownload(`https://www.youtube.com/watch?v=${args[2]}`, "./" + rootFolder + `/originVideo/videoplayback.mp4`),
            youtubeVideoDownloadWithFormat(`https://www.youtube.com/watch?v=${args[2]}`, "./" + rootFolder + `/originVideo/videoHightQuality.mp4`, '720p')
        ]);
    }
    if(!fs.existsSync("./" + rootFolder + `/originVideo/videoplayback1000000.mp4`)){
      console.log("Bắt đâu download và cut video")
        await Promise.all([
             execPromise(`ffmpeg -i "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoplayback.mp4" -c copy -map 0 -segment_time 360 -reset_timestamps 1 -f segment "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoplayback1%06d.mp4" -y`),
             execPromise(`ffmpeg -i "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoHightQuality.mp4" -c copy -map 0 -segment_time 360 -reset_timestamps 1 -f segment "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoHightQuality1%06d.mp4" -y`)
        ]);
    }
    await TaskReview(rootFolder, index, apiKeys, (videoID = " "),0,console);
    index = realIndex;
    fs.writeFileSync("./" + rootFolder +"/saveIndex.txt", `${index}`);
  }
...
*/
    ...
    await wait(60000)
    console.log("Bắt đầu extract image");
    await retry(extractImageNoOrderText, [rootFolder, index]);
    console.log("Hoàn thành extract");
