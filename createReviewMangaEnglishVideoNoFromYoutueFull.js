...
async function Run(apiKeys, startIndex = 0,condition=false,endIndex=10,colors,colorIndex, console = {}) {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if(!fs.existsSync("./" + rootFolder + `/originVideo/videoplayback1000000.mp4`)){
      console.log("Bắt đâu download và cut video")
        await Promise.all([
             execPromise(`ffmpeg -i "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoplayback.mp4" -c copy -map 0 -segment_time 600 -reset_timestamps 1 -f segment "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoplayback1%06d.mp4" -y`),
             execPromise(`ffmpeg -i "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoHightQuality.mp4" -c copy -map 0 -segment_time 600 -reset_timestamps 1 -f segment "/content/drive/MyDrive/0/tool_gemini/${rootFolder}/originVideo/videoHightQuality1%06d.mp4" -y`)
        ]);
    }
