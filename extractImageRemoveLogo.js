...
async function processTextFile(filePath) {
  ...
  async function extractFrameWidthBg(videoPath, startTime,endTime, lineIndex) {
    ...
    const backgroundPath = 'maskFfmpeg.png'
    const startExtractFrameTimeInSeconds = (parseTime(startTime) + parseTime(endTime))/2;
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ffmpeg(videoPath)
          ...
          .complexFilter(['overlay=x=165:y=83'])
/*    .complexFilter([
    '[0]scale=iw/3-20:ih/3-20[padded]',
    '[1][padded]scale2ref=w=iw+20:h=ih+20[bg][padded]',
    '[bg][padded]overlay=(W-w)/2:(H-h)/2[draw]',
    "[draw]drawtext=text='EngSub Commics':fontsize=25:fontcolor=white@0.02:x=(w-text_w)/2:y=(h-text_h)/2"
])*/
         /*
    .complexFilter([
    ...
    */
      ...
      console.log(`Frame extracted successfully to ${result}`);
/*
      await removeTextOnImageWithMask(outputImagePath)
        .then()
        .catch()
*/
      return result; // Optionally return the path for further use
