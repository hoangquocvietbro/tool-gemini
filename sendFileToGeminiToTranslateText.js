...
async function setUpGemini(rootFolder,videoIndex,ApiKey){
   ...
   fileManager = new GoogleAIFileManager(apiKey);
    ...
    const model = genAI.getGenerativeModel({
      ...
      const files = [
        fs.existsSync("./" +rootFolder+`/translatedSub/translate${videoIndex-1}.txt`)? await uploadToGemini("./" +rootFolder+`/translatedSub/translate${videoIndex-1}.txt`, "text/plain",fileManager): (fs.existsSync("./" +rootFolder+`/translatedSub/translate${videoIndex-2}.txt`)? await uploadToGemini("./" +rootFolder+`/translatedSub/translate${videoIndex-2}.txt`, "text/plain",fileManager):(        fs.existsSync("./" +rootFolder+`/translatedSub/translate${videoIndex-3}.txt`)? await uploadToGemini("./" +rootFolder+`/translatedSub/translate${videoIndex-3}.txt`, "text/plain",fileManager):await uploadToGemini(`./translate.txt`, "text/plain",fileManager))),
      ];
...
*/
      ...
      const result =
await chatSession.sendMessage(`Dựa vào nội dung phần trước ở file đã gửi ở trên hãy dịch nội dung ở dưới: \n\n${mangaName}\n ${SubText} \n\n sang *${language}*
...
fs.writeFileSync("./" +rootFolder+`/translatedSub/translate${videoIndex}.json`, textResult);
...
console.log(textResult);
                fs.writeFileSync("./" +rootFolder+`/translatedSub/translate${videoIndex}.txt`, textResult.replace(/\*|\[|\]/g, " "));
