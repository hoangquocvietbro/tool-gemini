...
if(fs.existsSync("/usr/bin/ffprobe")) ffmpeg.setFfmpegPath("/usr/bin/ffprobe");
        ...
        for(const imageFile of files)
            {
                ...
                promiseCreateVideoFromLine.push(new Promise(async(resolve,rejects)=>{
                    await createVideo(`./${rootFolder}/images/${videoIndex}/`+baseFileName+".png",`./${rootFolder}/mp3${language}/${videoIndex}/`+baseFileName+".mp3",`${videoIndex}/`+baseFileName)
                  resolve();
                  }
                  ));
...
async function concatAllOutputVideo(){
  ...
  const videoFiles = fs.readdirSync(`${rootFolder}/outputVideo${language}/${videoIndex}`).map(file => path.join(file));
  let list = ''; 
  for(const video of videoFiles)
    {
        
        
        list+= `file '../outputVideo${language}/${videoIndex}/${video.replace(/'/g, "\\'")}'\n`;
    }
    fs.writeFileSync(`${rootFolder}/videoFileList${language}/ListVideo${videoIndex}.txt`, list);
    const ffmpegProcess = ffmpeg()
    .input(`./${rootFolder}/videoFileList${language}/ListVideo${videoIndex}.txt`)
...
const ffmpegProcess = ffmpeg()
  ...
  .save(`${rootFolder}/outputVideo${language}/${outputVideo}.mp4`);
...
async function createVideo(imageListFile, audioListFile, outputVideo) {
  ...
  .complexFilter([
    ...
    '[draw]drawtext=text=`Review`:fontsize=30:fontcolor=white@0.2:x=(w-text_w)/2:y=(h-text_h)/2[image]',
...
async function runMakeVideo(RootFolder,VideoIndex,videoID){
  filename = RootFolder+`/VideoTranslated${language}/`+VideoIndex+"_"+videoID;
  rootFolder = RootFolder;
  videoIndex = VideoIndex;
   imagesDir = `${rootFolder}/images/${videoIndex}`;
  audioDir = `${rootFolder}/mp3${language}/${videoIndex}`;
  if(!fs.existsSync(`${rootFolder}/imageFileList${language}/${videoIndex}`)) fs.mkdirSync(`${rootFolder}/imageFileList${language}/${videoIndex}`)
    else{
      fs.rmdirSync(`${rootFolder}/imageFileList${language}/${videoIndex}`, { recursive: true, force: true })
      fs.mkdirSync(`${rootFolder}/imageFileList${language}/${videoIndex}`)
    }
  if(!fs.existsSync(`${rootFolder}/audioFileList${language}/${videoIndex}`)) fs.mkdirSync(`${rootFolder}/audioFileList${language}/${videoIndex}`)
    else{
      fs.rmdirSync(`${rootFolder}/audioFileList${language}/${videoIndex}`, { recursive: true, force: true })
      fs.mkdirSync(`${rootFolder}/audioFileList${language}/${videoIndex}`)
  }
  if(!fs.existsSync(`${rootFolder}/outputVideo${language}/${videoIndex}`)) fs.mkdirSync(`${rootFolder}/outputVideo${language}/${videoIndex}`)
    else{
      fs.rmdirSync(`${rootFolder}/outputVideo${language}/${videoIndex}`, { recursive: true, force: true })
      fs.mkdirSync(`${rootFolder}/outputVideo${language}/${videoIndex}`)
    }
