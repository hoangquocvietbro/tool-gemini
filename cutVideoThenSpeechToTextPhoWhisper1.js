...
async function extractAudio(rootPath, videoIndex) {
  return new Promise((resolve, reject) => {
    ffmpeg(`./${rootPath}/originVideo/videoplayback${videoIndex}.mp4`)
      .output(`./${rootPath}/originVideo/videoplayback${videoIndex}.mp4.mp3`)
      .noVideo()
      .on('start', (command) => {
        console.log(`Started: ${command}`);
      })
      .on('end', () => {      
        console.log('Created mp3 file from originVideo');
        resolve();
      })
      .on('error', (err) => {
        console.error(`Error creating mp3 file: ${err.message}`);
        reject(err);
      })
      .run();
  });
...
async function RunCutVideo(rootPath, videoIndex, ApiKey) {
  ...
  let inputVideoPath = "./" +rootPath+`/originVideo/videoplayback${videoIndex}.mp4.mp3`;
      if(!fs.existsSync(inputVideoPath)) {

    await extractAudio(rootPath, videoIndex).then().catch();  
  }
  ...
  const processTimestamps = async () => {
    ...
    for (let i = 0; i < timestamps.length; i++) {
      ...
      const startTime = parseTime(start)+1;
      let endTime = parseTime(end)+1;
      ...
      if(processsegmentPromise.length%80==0||i == timestamps.length-1)  {
         await Promise.all(processsegmentPromise);
      }
  ...
  async function processsegmentToText(inputVideoPath, startTime, endTime, outputFilePath,rootPath, videoIndex,threadApi,start, end, isStopTryAgain=false){
    try{

      await cutVideo(inputVideoPath, startTime, endTime, outputFilePath);
      let result  = "";
      await runPythonPhoWhisperScript(outputFilePath)
                        .then(text => {result=text;})
                        .catch(error => {console.error('Error:', error);});
      if(result==""&&isStopTryAgain==false){
           return await processsegmentToText(inputVideoPath, startTime, endTime, outputFilePath,rootPath, videoIndex,threadApi,start, end, true)
      }
...
async function runPythonPhoWhisperScript(filePath) {
  return await new  Promise((resolve, reject) => {
                       exec(`curl -X POST http://localhost:5000/transcribe -H "Content-Type: application/json" -d '{\"file_path\": \"${filePath}\"}'`, (error, stdout, stderr) => {
    resolve(stdout.trim());
  });
  });
}
