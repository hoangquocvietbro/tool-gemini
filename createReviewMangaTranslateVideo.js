...
const sendFileToGeminiToTranslateText = require("./sendFileEnglishToGeminiToTranslateText");
const extractImage = require("./extractImage");
const extractImageAndMakeMp3 = require("./extractImageAndMakeMp3Language");
const createVideoFromImagesAndMp3Folder = require("./createVideoFromImagesAndMp3Folder");
const createVideoFromImagesAndMp3FolderZoom = require("./createVideoFromImagesAndMp3FolderZoomLanguage");
...
async function TaskReview(rootFolder, index, apiKeys,url, videoID=" ",IndexApi=0) {
  try {
    ...
    [
      `./mp3${language}`,
      `./translatedSub${language}`,
      `./VideoTranslated${language}`,
      `./imageFileList${language}`,
      `./audioFileList${language}`,
      `./videoFileList${language}`,
      `./outputVideo${language}`
    ].forEach((dir, index, dirs) => {
      if (!fs.existsSync("./" + rootFolder + "/" + dir)) {
        fs.mkdirSync("./" + rootFolder + "/" + dir);
      }
    ...
    await retry(createVideoFromImagesAndMp3FolderZoom, [rootFolder, index, videoID]);
